...
LOGDIR = ["MAPPO2_" + str(SEED) + "/model1", "MAPPO2_" + str(SEED) + "/model2", "MAPPO2_" + str(SEED) + "/model3"]
...
class TeamVolleyBotSelfPlayEnv(TeamVolleyBot):
    def __init__(self, n_agents=3):
        ...
        self.best_model_filename = [None, None, None]
...
class SelfPlayCallback:
    def __init__(self,
                 ...
                 log_path: list = [None, None, None],
                 best_model_save_path: list = [None, None, None],
    ...
    def check(self):
        ...
        self.n_calls += 1  # check eval_freq次才返回一次true
    ...
    def on_step(self, episode_rewards, episode_lengths) -> bool:
        ...
        if self.verbose > 0:
            print("Eval num_timesteps={}, "
                  "episode_reward={:.2f} +/- {:.2f}".format(self.models.agents[0].num_timesteps, mean_reward,
                                                            std_reward))
        ...
        if self.last_mean_len >= self.upgr_len:
            ...
            if self.env.update and (self.n_calls % self.eval_freq == 0):
                ...
                self.new_depth = True
                for i in range(self.n_agents):
                    self.models.agents[i].save(os.path.join(self.best_model_save_path[i],
                                                            "increment_model_" + str(int(self.env.world.depth)).zfill(
                                                                4) + ".zip"))
...
def Train():
    ...
    models.learn(total_timesteps, eval_callback=eval_callback, save_dir=SAVE_DIR, save_path=LOGDIR,
                 save_interval=50)  # 目前总共是2000多update
